...
4. Auto-tracking in `smart_turn(start/end)` when `DEVCTX_WORKFLOW_TRACKING=true`

**No breaking changes:**
- Opt-in via `DEVCTX_WORKFLOW_TRACKING=true`
- Graceful degradation if disabled
- No impact on existing tool metrics
- Workflow tracking stays dormant unless explicitly enabled by environment variable
